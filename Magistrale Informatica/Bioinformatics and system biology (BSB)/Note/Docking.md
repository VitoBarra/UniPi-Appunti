...
L’**affidabilità del docking** valuta quanto un algoritmo sia capace di ricostruire pose di legame realistiche. Il criterio operativo consiste nel confrontare pose predette con pose sperimentali note (tipicamente da cristallografia), misurando lo scostamento geometrico tramite **RMSD** calcolato sugli atomi del ligando (di norma atomi pesanti) dopo sovrapposizione rigida del recettore.

Nel test di affidabilità si parte da un **complesso ligando–proteina cristallografico**, lo si separa in recettore e ligando, quindi:
- il ligando viene sottoposto a **ricerca conformazionale** (per generare conformeri accessibili)  
- il ligando (o i suoi conformeri) viene poi sottoposto a **docking** nel sito attivo del recettore  
- la posa predetta viene confrontata con la posa cristallografica tramite RMSD  

Se l’RMSD è basso, il docking ha ricostruito correttamente l’assetto sperimentale; se l’RMSD è alto, la posa predetta è significativamente diversa.  
![[IMG - affidabilita docking self docking.png]]

### Self-docking
Il **self-docking** testa la capacità dell’algoritmo di riprodurre la posa di un ligando quando viene dockato nella stessa proteina da cui la posa sperimentale è stata ottenuta. Il flusso è:
- si prende il complesso cristallografico (ligando + proteina)  
- si rimuove il ligando dalla cavità  
- si re-docka lo stesso ligando nella stessa proteina  
- si calcola l’RMSD tra posa predetta e posa cristallografica  

Il self-docking misura soprattutto la correttezza del posizionamento del ligando in un binding site “già adatto” a quel ligando, e tende quindi a rappresentare un test relativamente favorevole.  
![[IMG - affidabilita docking self docking.png]]

### Self-docking multiplo
