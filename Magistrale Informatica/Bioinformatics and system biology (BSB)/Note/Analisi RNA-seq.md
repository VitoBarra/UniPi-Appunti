...
Una prima modellizzazione naturale del rapporto **media-varianza** dei conteggi RNA-seq è usando una [[Variabili Aleatorie Notevoli - Poisson|distribuzione di Poisson]]:$$Y_{gi}\sim \text{Poisson}(\lambda_{gi})$$dove $g$ indica il gene, $i$ il campione e $\lambda_{gi}$ il [[Variabili aleatoria - Momenti o Valore atteso|numero atteso]] di reads per quel gene in quel campione, determinato da livello di espressione, dimensione della libreria e altri fattori globali.
