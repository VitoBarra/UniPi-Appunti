...
Una distinzione fondamentale riguarda i **transcription factors (TF)**. Un **transcription factor** è un gene che codifica una [[Proteine|proteina]] in grado di legarsi al [[DNA Struttura e funzionalità|DNA]] e regolare la trascrizione di altri geni.
Separare i geni in:
- **regolatori** (transcription factors)
- **target genes**
riduce significativamente lo spazio delle possibili interazioni da stimare. Se $T$ è l’insieme dei **transcription factors** e $G$ l’insieme di tutti i geni, le interazioni candidate diventano$$T \times G$$invece di $G\times G$. Questo vincolo migliora la qualità dell’inferenza e riduce la complessità computazionale dei metodi.

Un altro tipo di dato molto informativo deriva da esperimenti di **knock-out** o **knock-down**, in cui l’attività di un gene viene eliminata o ridotta. Confrontando il profilo di espressione nel sistema **wild-type** con quello ottenuto dopo il knock-out, è possibile identificare geni influenzati da quello disattivato. Se $x_j^{WT}$ rappresenta l’espressione del gene $j$ nel wild-type e $x_j^{KO(i)}$ quella osservata dopo il knock-out del gene $i$, una variazione significativa$$x_j^{KO(i)} \neq x_j^{WT}$$indica che il gene $j$ è influenzato direttamente o indirettamente da $i$.

Un’altra fonte importante di informazione è costituita dagli esperimenti [[ChIP-seq]].
...
- **metodi information-theoretic**, basati su misure di dipendenza come la mutual information ad esempio [[GRN - inferenza - ARACNE|ARACNE]]
