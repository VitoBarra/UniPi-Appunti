...
Applicandola con $Y=\frac{p(x,z \mid \theta)}{q(z \mid \phi)}$ si ottiene: $$\log p(x \mid \theta)\geq \mathbb{E}_q\left[\log \frac{p(x,z \mid \theta)}{q(z \mid \phi)}\right]$$ cioè: $$\log p(x \mid \theta)\geq \mathbb{E}_q[\log p(x,z \mid \theta)]-\mathbb{E}_q[\log q(z \mid \phi)]$$
