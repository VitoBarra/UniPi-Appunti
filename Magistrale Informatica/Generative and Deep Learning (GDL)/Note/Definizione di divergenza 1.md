...
Dato un campo vettoriale $\mathbf{F} = (F_1, F_2, \dots, F_n)$

definito in $\mathbb{R}^n$, la divergenza è definita come la somma delle derivate parziali delle componenti del campo rispetto alle rispettive coordinate spaziali.

$$\nabla \cdot \mathbf{F} = \sum_{i=1}^{n} \frac{\partial F_i}{\partial x_i}$$

Nel caso tridimensionale, per un campo $\mathbf{F} = (F_x, F_y, F_z)$
la divergenza assume la forma$$\nabla \cdot \mathbf{F} =
