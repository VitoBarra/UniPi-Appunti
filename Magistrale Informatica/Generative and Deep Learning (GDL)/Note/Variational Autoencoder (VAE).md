...
---
I **Variational Autoencoders** (**VAE**) trasformano l'idea degli [[Autoencoders (AE)|autoencoders]] in un [[Latent Model|modello latente]] probabilistico e [[Modelli Generativi|generativo]].

In un VAE, invece, il codice latente non è un singolo vettore deterministico, ma una variabile aleatoria. Il modello definisce:
- un **prior** sulle variabili latenti, di solito $p(z) = \mathcal{N}(0, I)$;
- un **decoder probabilistico** $p_{\theta}(x \mid z)$;
...
Le variabili latenti $z$ rappresentano fattori nascosti di variazione del dato, ad esempio identità, stile, rotazione, posa o dimensione.

## Likelihood marginale
...
Il problema è che questo integrale è di solito intrattabile quando il decoder è una rete neurale e $z$ è continuo.
...
è intrattabile, perché richiede proprio la marginal likelihood $p_{\theta}(x)$ al denominatore.

## Encoder variazionale
...
L'encoder non produce direttamente $z$, ma i parametri della distribuzione:
$$
\mu_{\phi}(x), \sigma_{\phi}(x)
$$

Da questa distribuzione si campiona poi un valore latente $z$ da passare al decoder.


![[IMG - Architettura Variational Autoencoder (VAE).png]]
