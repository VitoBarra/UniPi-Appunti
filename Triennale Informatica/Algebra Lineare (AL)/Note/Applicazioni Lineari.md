...
_allora_ Un’_applicazione lineare_ è una [[Funzioni|funzione]] fra [[Spazi Vettoriali|spazi vettoriali]]$$f:V\rightarrow W$$che soddisfano i seguenti assiomi
- $f(0)=0$
- $f (v + w) = f (v ) + f (w)$ per ogni $v,w\in V$
- $f (\lambda v ) = \lambda f (v)$  per ogni $v \in V$   e $\lambda \in\mathbb{K}$
una proprietà di questi assiomi è che 
- La _composizione_ di **_applicazioni lineari_** resta un **_applicazone lineare_**
